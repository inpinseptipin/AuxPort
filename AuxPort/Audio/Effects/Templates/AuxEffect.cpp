...
float AuxPort::Audio::Synthesizer::midiToFreq(uint32_t midiNote)
{
	...
	return referenceFrequency * powf(2, (static_cast<float>(midiNote) - referenceMidiNote) / 12.0f);
}
...
void AuxPort::Audio::Synthesizer::configureMidiToFreq(uint32_t midiNote, float frequency)
{
	...
	AuxAssert(frequency * powf(2, (static_cast<float>(0) - midiNote) / 12.0f > 0), "Invalid reference midi and frequency combination");
	AuxAssert(frequency * powf(2, (static_cast<float>(127) - midiNote) / 12.0f < 20000), "Invalid reference midi and frequency combination");
